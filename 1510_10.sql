...
преимущества
модульность, удобство сопровождения
скорость 
трафик
безопасность при передаче по каналам, скрытие исх. кода
удобство  настр безопасности и прав пользов
...
alter proc average
 @x float = 0,
 @y float = 0, 
 @res_arithm float output,
 @res_geom float output
with encryption, --скрытие исх кода
recompile --рекомпиляция при каждом зауске
...
set @temp= @temp/2.0
set @res_arithm = @temp
set @res_geom = sqrt(@x*@y)

if @res_arithm is null  or @res_geom is null return -1
else
return 0 --возвращаем код заверщения, но не результат 
end
...
declare @t float, @t2 float, @r float, @r2 float, @codeee int, @a int
set @t=45.6;
select @t2 = AVG(стаж) from сотрудники

--запуск!!
--exec average @t, @t2, @r output, @r2 output --не читаем код завершения
--exec @codeee = average @a, @t2, @r output, @r2 output -- читаем код завершения
--exec @codeee = average  @res_arithm = @r output, @y=4, @x=8,  @res_geom=@r2 output --непозиционное связывание 
exec @codeee = average  @res_arithm = @r output,  @x=8,  @res_geom=@r2 output --непозиционное связывание 



/*print 'res is '+ cast(@r as varchar(10)) +' and '+
 cast(@r2 as varchar(10)) + 
 ' return code is '+ cast(@codeee as varchar(10))*/

print 'res is '+ isnull(cast(@r as varchar(10)), ' ' )+' and '+
 isnull(cast(@r2 as varchar(10)),' ') + 
 ' return code is '+ isnull(cast(@codeee as varchar(10)), ' ')
 
 raiserror ('произошла ошибка!!!! ', 20, 120 ) with log;
 
 
 create proc get_coeff
 @id int,
 @coeff float output
 with encryption
 as
 ...
 declare @salescount int, @exp int, @avgprice float
 
 select @salescount = COUNT(*) from sales where id_sellers = @id
 select @exp = experience from sellers where id_sellers = @id
 select @avgprice = AVG(price) from goods 
 
 set @coeff = (@salescount*@exp)/@avgprice
 return 0
 end
 
 exec get_coeff 4, @res output
 ...
 create trigger name_tr
 on table1
 for | after | instead of actions 
 with options 
 as
 begin
 end
 ...
 --inserted
 --deleted
 --updated нет такой псевдотаблицы!!!
 ...
 as
 begin
 print 'Добавили или удалили должность'
 end
 
 insert должности values 
 ('экс клининг-менеджер',45000), 
 ('экс клининг-директор',45000)
 
 
 
 alter trigger trigga1
 on Должности
 after insert, delete 
 ...
 begin
 print 'удалили должность'
 end
 if (select COUNT(*) from inserted)>0
 begin
  print 'добавили должность'
  
  if (select SUM(Базовый_оклад) from inserted)>60000
  begin
	--insert into registrator values(.......)
	print 'Добавили должности с большим окладом'
  end
 end
 ...
 sp_configure recursive_triggers, 1
 sp_configure recursive_triggers, 0
 
 drop trigger trigga1
 
 alter table Должности
 enable trigger trigga1
 
 alter table Должности
 disable trigger trigga1
 ...
 неяvные implicit
 явные  explicit
 автозавершающиеся autocommiting
...
select * from Должности
update Должности set Название_должности = SUBSTRING(Название_должности, 5, LEN(Название_должности)-4)
where ID_Должности in (9,10)
insert Должности values ('какая-то ещё должность', 1500000)
...
lost updates			U  <s>U</s>     read uncommitted -- чтение неподтв данных
dirty read				U  <s>R</s>     read committed  --чтение подтв данных
nonrepeatable read		R  <s>U</s>		repeatable read --повторяемое чтение
....									snapshot --уровень снимка
phantom values			keyset*			serializable --сериализованые операции 

set transaction isolation level read uncommitted --меняем уровень изо


commit
select * from Должности
update Должности set Базовый_оклад = Базовый_оклад*0.9
where ID_Должности in (10)
rollback
...
--влиять можно через подсказки оптимизатору
...
эскалация блокировок - повышение степени гранулированности 
...
--индексы 
кластерные - реально упорядочивают данные          1
некластерные - реально не упорядочивают данные		<255 

create /*unique*/ nonclustered  index ind1 
on Должности (Название_должности asc)
with drop_existing, fillfactor=50, pad_index 

create clustered index ind2 
on Должности (Название_должности asc)

sp_help Должности
select * from sys.indexes order by name --where object_id = ()

drop index Должности.ind1


Alter index ind1 on Должности reorganize
