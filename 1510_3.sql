...
(
 ...
 foreign key references Должности(ID_Должности) on delete cascade default 1 							-- создаем и поле и ключ
 
 alter table Сотрудники 																				-- создаем поле
 add ID_Отдела tinyint
 
 alter table Сотрудники
 add constraint ref_dep foreign key (ID_Отдела) references Отделы(ID_Отдела)on delete cascade			-- создаем ключ
